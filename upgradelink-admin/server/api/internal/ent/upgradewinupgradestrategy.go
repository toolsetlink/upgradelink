...
// UpgradeWinUpgradeStrategy is the model entity for the UpgradeWinUpgradeStrategy schema.
type UpgradeWinUpgradeStrategy struct {
	...
	WinVersionID int `json:"win_version_id,omitempty"`
	// 升级任务开始时间
...
func (*UpgradeWinUpgradeStrategy) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case upgradewinupgradestrategy.FieldID, upgradewinupgradestrategy.FieldCompanyID, upgradewinupgradestrategy.FieldEnable, upgradewinupgradestrategy.FieldWinID, upgradewinupgradestrategy.FieldWinVersionID, upgradewinupgradestrategy.FieldUpgradeType, upgradewinupgradestrategy.FieldUpgradeDevType, upgradewinupgradestrategy.FieldUpgradeVersionType, upgradewinupgradestrategy.FieldIsGray, upgradewinupgradestrategy.FieldIsFlowLimit, upgradewinupgradestrategy.FieldIsDel:
			values[i] = new(sql.NullInt64)
		case upgradewinupgradestrategy.FieldName, upgradewinupgradestrategy.FieldDescription, upgradewinupgradestrategy.FieldPromptUpgradeContent, upgradewinupgradestrategy.FieldUpgradeDevData, upgradewinupgradestrategy.FieldUpgradeVersionData, upgradewinupgradestrategy.FieldGrayData, upgradewinupgradestrategy.FieldFlowLimitData:
			values[i] = new(sql.NullString)
		case upgradewinupgradestrategy.FieldBeginDatetime, upgradewinupgradestrategy.FieldEndDatetime, upgradewinupgradestrategy.FieldCreateAt, upgradewinupgradestrategy.FieldUpdateAt:
...
// to the UpgradeWinUpgradeStrategy fields.
func (_m *UpgradeWinUpgradeStrategy) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case upgradewinupgradestrategy.FieldID:
		...
		case upgradewinupgradestrategy.FieldWinVersionID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field win_version_id", values[i])
			} else if value.Valid {
				_m.WinVersionID = int(value.Int64)
			}
...
// Value returns the ent.Value that was dynamically selected and assigned to the UpgradeWinUpgradeStrategy.
// This includes values selected through modifiers, order, etc.
func (_m *UpgradeWinUpgradeStrategy) Value(name string) (ent.Value, error) {
	return _m.selectValues.Get(name)
}

// Update returns a builder for updating this UpgradeWinUpgradeStrategy.
// Note that you need to call UpgradeWinUpgradeStrategy.Unwrap() before calling this method if this UpgradeWinUpgradeStrategy
// was returned from a transaction, and the transaction was committed or rolled back.
func (_m *UpgradeWinUpgradeStrategy) Update() *UpgradeWinUpgradeStrategyUpdateOne {
	return NewUpgradeWinUpgradeStrategyClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the UpgradeWinUpgradeStrategy entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *UpgradeWinUpgradeStrategy) Unwrap() *UpgradeWinUpgradeStrategy {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("ent: UpgradeWinUpgradeStrategy is not a transactional entity")
	}
...
func (_m *UpgradeWinUpgradeStrategy) String() string {
	var builder strings.Builder
	builder.WriteString("UpgradeWinUpgradeStrategy(")
	...
	builder.WriteString(", ")
	builder.WriteString("win_version_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.WinVersionID))
	builder.WriteString(", ")
...
// UpgradeWinUpgradeStrategies is a parsable slice of UpgradeWinUpgradeStrategy.
type UpgradeWinUpgradeStrategies []*UpgradeWinUpgradeStrategy
