// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"errors"
	"fmt"
	"time"
	"upgradelink-admin/server/api/internal/ent/fmscloudfile"

	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
)

// FmsCloudFileCreate is the builder for creating a FmsCloudFile entity.
type FmsCloudFileCreate struct {
	config
	mutation *FmsCloudFileMutation
	hooks    []Hook
}

// SetCreatedAt sets the "created_at" field.
func (_c *FmsCloudFileCreate) SetCreatedAt(v time.Time) *FmsCloudFileCreate {
	_c.mutation.SetCreatedAt(v)
	return _c
}

// SetNillableCreatedAt sets the "created_at" field if the given value is not nil.
func (_c *FmsCloudFileCreate) SetNillableCreatedAt(v *time.Time) *FmsCloudFileCreate {
	if v != nil {
		_c.SetCreatedAt(*v)
	}
	return _c
}

// SetUpdatedAt sets the "updated_at" field.
func (_c *FmsCloudFileCreate) SetUpdatedAt(v time.Time) *FmsCloudFileCreate {
	_c.mutation.SetUpdatedAt(v)
	return _c
}

// SetNillableUpdatedAt sets the "updated_at" field if the given value is not nil.
func (_c *FmsCloudFileCreate) SetNillableUpdatedAt(v *time.Time) *FmsCloudFileCreate {
	if v != nil {
		_c.SetUpdatedAt(*v)
	}
	return _c
}

// SetState sets the "state" field.
func (_c *FmsCloudFileCreate) SetState(v bool) *FmsCloudFileCreate {
	_c.mutation.SetState(v)
	return _c
}

// SetNillableState sets the "state" field if the given value is not nil.
func (_c *FmsCloudFileCreate) SetNillableState(v *bool) *FmsCloudFileCreate {
	if v != nil {
		_c.SetState(*v)
	}
	return _c
}

// SetName sets the "name" field.
func (_c *FmsCloudFileCreate) SetName(v string) *FmsCloudFileCreate {
	_c.mutation.SetName(v)
	return _c
}

// SetURL sets the "url" field.
func (_c *FmsCloudFileCreate) SetURL(v string) *FmsCloudFileCreate {
	_c.mutation.SetURL(v)
	return _c
}

// SetSize sets the "size" field.
func (_c *FmsCloudFileCreate) SetSize(v uint64) *FmsCloudFileCreate {
	_c.mutation.SetSize(v)
	return _c
}

// SetMd5 sets the "md5" field.
func (_c *FmsCloudFileCreate) SetMd5(v string) *FmsCloudFileCreate {
	_c.mutation.SetMd5(v)
	return _c
}

// SetNillableMd5 sets the "md5" field if the given value is not nil.
func (_c *FmsCloudFileCreate) SetNillableMd5(v *string) *FmsCloudFileCreate {
	if v != nil {
		_c.SetMd5(*v)
	}
	return _c
}

// SetFileType sets the "file_type" field.
func (_c *FmsCloudFileCreate) SetFileType(v uint8) *FmsCloudFileCreate {
	_c.mutation.SetFileType(v)
	return _c
}

// SetUserID sets the "user_id" field.
func (_c *FmsCloudFileCreate) SetUserID(v string) *FmsCloudFileCreate {
	_c.mutation.SetUserID(v)
	return _c
}

// SetID sets the "id" field.
func (_c *FmsCloudFileCreate) SetID(v string) *FmsCloudFileCreate {
	_c.mutation.SetID(v)
	return _c
}

// Mutation returns the FmsCloudFileMutation object of the builder.
func (_c *FmsCloudFileCreate) Mutation() *FmsCloudFileMutation {
	return _c.mutation
}

// Save creates the FmsCloudFile in the database.
func (_c *FmsCloudFileCreate) Save(ctx context.Context) (*FmsCloudFile, error) {
	return withHooks(ctx, _c.sqlSave, _c.mutation, _c.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (_c *FmsCloudFileCreate) SaveX(ctx context.Context) *FmsCloudFile {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *FmsCloudFileCreate) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *FmsCloudFileCreate) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}

// check runs all checks and user-defined validators on the builder.
func (_c *FmsCloudFileCreate) check() error {
	if _, ok := _c.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`ent: missing required field "FmsCloudFile.name"`)}
	}
	if _, ok := _c.mutation.URL(); !ok {
		return &ValidationError{Name: "url", err: errors.New(`ent: missing required field "FmsCloudFile.url"`)}
	}
	if _, ok := _c.mutation.Size(); !ok {
		return &ValidationError{Name: "size", err: errors.New(`ent: missing required field "FmsCloudFile.size"`)}
	}
	if _, ok := _c.mutation.FileType(); !ok {
		return &ValidationError{Name: "file_type", err: errors.New(`ent: missing required field "FmsCloudFile.file_type"`)}
	}
	if _, ok := _c.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`ent: missing required field "FmsCloudFile.user_id"`)}
	}
	return nil
}

func (_c *FmsCloudFileCreate) sqlSave(ctx context.Context) (*FmsCloudFile, error) {
	if err := _c.check(); err != nil {
		return nil, err
	}
	_node, _spec := _c.createSpec()
	if err := sqlgraph.CreateNode(ctx, _c.driver, _spec); err != nil {
		if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return nil, err
	}
	if _spec.ID.Value != nil {
		if id, ok := _spec.ID.Value.(string); ok {
			_node.ID = id
		} else {
			return nil, fmt.Errorf("unexpected FmsCloudFile.ID type: %T", _spec.ID.Value)
		}
	}
	_c.mutation.id = &_node.ID
	_c.mutation.done = true
	return _node, nil
}

func (_c *FmsCloudFileCreate) createSpec() (*FmsCloudFile, *sqlgraph.CreateSpec) {
	var (
		_node = &FmsCloudFile{config: _c.config}
		_spec = sqlgraph.NewCreateSpec(fmscloudfile.Table, sqlgraph.NewFieldSpec(fmscloudfile.FieldID, field.TypeString))
	)
	if id, ok := _c.mutation.ID(); ok {
		_node.ID = id
		_spec.ID.Value = id
	}
	if value, ok := _c.mutation.CreatedAt(); ok {
		_spec.SetField(fmscloudfile.FieldCreatedAt, field.TypeTime, value)
		_node.CreatedAt = value
	}
	if value, ok := _c.mutation.UpdatedAt(); ok {
		_spec.SetField(fmscloudfile.FieldUpdatedAt, field.TypeTime, value)
		_node.UpdatedAt = value
	}
	if value, ok := _c.mutation.State(); ok {
		_spec.SetField(fmscloudfile.FieldState, field.TypeBool, value)
		_node.State = value
	}
	if value, ok := _c.mutation.Name(); ok {
		_spec.SetField(fmscloudfile.FieldName, field.TypeString, value)
		_node.Name = value
	}
	if value, ok := _c.mutation.URL(); ok {
		_spec.SetField(fmscloudfile.FieldURL, field.TypeString, value)
		_node.URL = value
	}
	if value, ok := _c.mutation.Size(); ok {
		_spec.SetField(fmscloudfile.FieldSize, field.TypeUint64, value)
		_node.Size = value
	}
	if value, ok := _c.mutation.Md5(); ok {
		_spec.SetField(fmscloudfile.FieldMd5, field.TypeString, value)
		_node.Md5 = value
	}
	if value, ok := _c.mutation.FileType(); ok {
		_spec.SetField(fmscloudfile.FieldFileType, field.TypeUint8, value)
		_node.FileType = value
	}
	if value, ok := _c.mutation.UserID(); ok {
		_spec.SetField(fmscloudfile.FieldUserID, field.TypeString, value)
		_node.UserID = value
	}
	return _node, _spec
}

// FmsCloudFileCreateBulk is the builder for creating many FmsCloudFile entities in bulk.
type FmsCloudFileCreateBulk struct {
	config
	err      error
	builders []*FmsCloudFileCreate
}

// Save creates the FmsCloudFile entities in the database.
func (_c *FmsCloudFileCreateBulk) Save(ctx context.Context) ([]*FmsCloudFile, error) {
	if _c.err != nil {
		return nil, _c.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(_c.builders))
	nodes := make([]*FmsCloudFile, len(_c.builders))
	mutators := make([]Mutator, len(_c.builders))
	for i := range _c.builders {
		func(i int, root context.Context) {
			builder := _c.builders[i]
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				mutation, ok := m.(*FmsCloudFileMutation)
				if !ok {
					return nil, fmt.Errorf("unexpected mutation type %T", m)
				}
				if err := builder.check(); err != nil {
					return nil, err
				}
				builder.mutation = mutation
				var err error
				nodes[i], specs[i] = builder.createSpec()
				if i < len(mutators)-1 {
					_, err = mutators[i+1].Mutate(root, _c.builders[i+1].mutation)
				} else {
					spec := &sqlgraph.BatchCreateSpec{Nodes: specs}
					// Invoke the actual operation on the latest mutation in the chain.
					if err = sqlgraph.BatchCreate(ctx, _c.driver, spec); err != nil {
						if sqlgraph.IsConstraintError(err) {
							err = &ConstraintError{msg: err.Error(), wrap: err}
						}
					}
				}
				if err != nil {
					return nil, err
				}
				mutation.id = &nodes[i].ID
				mutation.done = true
				return nodes[i], nil
			})
			for i := len(builder.hooks) - 1; i >= 0; i-- {
				mut = builder.hooks[i](mut)
			}
			mutators[i] = mut
		}(i, ctx)
	}
	if len(mutators) > 0 {
		if _, err := mutators[0].Mutate(ctx, _c.builders[0].mutation); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

// SaveX is like Save, but panics if an error occurs.
func (_c *FmsCloudFileCreateBulk) SaveX(ctx context.Context) []*FmsCloudFile {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *FmsCloudFileCreateBulk) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *FmsCloudFileCreateBulk) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}
