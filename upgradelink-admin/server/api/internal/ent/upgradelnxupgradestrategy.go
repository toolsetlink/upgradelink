...
// UpgradeLnxUpgradeStrategy is the model entity for the UpgradeLnxUpgradeStrategy schema.
type UpgradeLnxUpgradeStrategy struct {
	...
	LnxVersionID int `json:"lnx_version_id,omitempty"`
	// 升级任务开始时间
...
func (*UpgradeLnxUpgradeStrategy) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case upgradelnxupgradestrategy.FieldID, upgradelnxupgradestrategy.FieldCompanyID, upgradelnxupgradestrategy.FieldEnable, upgradelnxupgradestrategy.FieldLnxID, upgradelnxupgradestrategy.FieldLnxVersionID, upgradelnxupgradestrategy.FieldUpgradeType, upgradelnxupgradestrategy.FieldUpgradeDevType, upgradelnxupgradestrategy.FieldUpgradeVersionType, upgradelnxupgradestrategy.FieldIsGray, upgradelnxupgradestrategy.FieldIsFlowLimit, upgradelnxupgradestrategy.FieldIsDel:
			values[i] = new(sql.NullInt64)
		case upgradelnxupgradestrategy.FieldName, upgradelnxupgradestrategy.FieldDescription, upgradelnxupgradestrategy.FieldPromptUpgradeContent, upgradelnxupgradestrategy.FieldUpgradeDevData, upgradelnxupgradestrategy.FieldUpgradeVersionData, upgradelnxupgradestrategy.FieldGrayData, upgradelnxupgradestrategy.FieldFlowLimitData:
			values[i] = new(sql.NullString)
		case upgradelnxupgradestrategy.FieldBeginDatetime, upgradelnxupgradestrategy.FieldEndDatetime, upgradelnxupgradestrategy.FieldCreateAt, upgradelnxupgradestrategy.FieldUpdateAt:
...
// to the UpgradeLnxUpgradeStrategy fields.
func (_m *UpgradeLnxUpgradeStrategy) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case upgradelnxupgradestrategy.FieldID:
		...
		case upgradelnxupgradestrategy.FieldLnxID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.LnxID = int(value.Int64)
			}
		case upgradelnxupgradestrategy.FieldLnxVersionID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field lnx_version_id", values[i])
			} else if value.Valid {
				_m.LnxVersionID = int(value.Int64)
			}
...
// Value returns the ent.Value that was dynamically selected and assigned to the UpgradeLnxUpgradeStrategy.
// This includes values selected through modifiers, order, etc.
func (_m *UpgradeLnxUpgradeStrategy) Value(name string) (ent.Value, error) {
	return _m.selectValues.Get(name)
}

// Update returns a builder for updating this UpgradeLnxUpgradeStrategy.
// Note that you need to call UpgradeLnxUpgradeStrategy.Unwrap() before calling this method if this UpgradeLnxUpgradeStrategy
// was returned from a transaction, and the transaction was committed or rolled back.
func (_m *UpgradeLnxUpgradeStrategy) Update() *UpgradeLnxUpgradeStrategyUpdateOne {
	return NewUpgradeLnxUpgradeStrategyClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the UpgradeLnxUpgradeStrategy entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *UpgradeLnxUpgradeStrategy) Unwrap() *UpgradeLnxUpgradeStrategy {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("ent: UpgradeLnxUpgradeStrategy is not a transactional entity")
	}
...
func (_m *UpgradeLnxUpgradeStrategy) String() string {
	var builder strings.Builder
	builder.WriteString("UpgradeLnxUpgradeStrategy(")
	...
	builder.WriteString(fmt.Sprintf("%v", _m.LnxID))
	builder.WriteString(", ")
	builder.WriteString("lnx_version_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.LnxVersionID))
	builder.WriteString(", ")
...
// UpgradeLnxUpgradeStrategies is a parsable slice of UpgradeLnxUpgradeStrategy.
type UpgradeLnxUpgradeStrategies []*UpgradeLnxUpgradeStrategy
