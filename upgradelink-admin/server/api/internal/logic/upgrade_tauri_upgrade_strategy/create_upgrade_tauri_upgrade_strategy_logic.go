...
type CreateUpgradeTauriUpgradeStrategyLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewCreateUpgradeTauriUpgradeStrategyLogic(ctx context.Context, svcCtx *svc.ServiceContext) *CreateUpgradeTauriUpgradeStrategyLogic {
	return &CreateUpgradeTauriUpgradeStrategyLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *CreateUpgradeTauriUpgradeStrategyLogic) CreateUpgradeTauriUpgradeStrategy(req *types.UpgradeTauriUpgradeStrategyInfo) (*types.BaseMsgResp, error) {

	// 效验请求参数数据
	err := l.CheckCreateUpgradeTauriUpgradeStrategy(req)
	if err != nil {
		return nil, err
	}
	...
	if err := entx.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		_, err = l.svcCtx.DB.UpgradeTauriUpgradeStrategy.Create().
...
func (l *CreateUpgradeTauriUpgradeStrategyLogic) CheckCreateUpgradeTauriUpgradeStrategy(req *types.UpgradeTauriUpgradeStrategyInfo) error {
	// 判断是否重复
	...
	predicates = append(predicates, upgradetauriupgradestrategy.CompanyIDEQ(companyctx.GetCompanyIDFromCtx(l.ctx)))
