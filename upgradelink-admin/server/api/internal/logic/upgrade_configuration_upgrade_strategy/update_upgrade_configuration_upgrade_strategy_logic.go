...
type UpdateUpgradeConfigurationUpgradeStrategyLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewUpdateUpgradeConfigurationUpgradeStrategyLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateUpgradeConfigurationUpgradeStrategyLogic {
	return &UpdateUpgradeConfigurationUpgradeStrategyLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *UpdateUpgradeConfigurationUpgradeStrategyLogic) UpdateUpgradeConfigurationUpgradeStrategy(req *types.UpgradeConfigurationUpgradeStrategyInfo) (*types.BaseMsgResp, error) {

	// 效验请求参数数据
	err := l.CheckUpdateUpgradeConfigurationUpgradeStrategy(req)
	if err != nil {
		return nil, err
	}

	// 获取策略信息
	strategyData, err := l.svcCtx.DB.UpgradeConfigurationUpgradeStrategy.Get(l.ctx, *req.Id)
	if err != nil {
	...
	if err := entx.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		for i := 0; i < len(flowLimitIds); i++ {
			err = l.svcCtx.DB.UpgradeConfigurationUpgradeStrategyFlowLimitStrategy.Update().
				Where(upgradeconfigurationupgradestrategyflowlimitstrategy.IDEQ(flowLimitIds[i])).
				SetNotNilIsDel(&intDelTrue).
				Exec(l.ctx)
			if err != nil {
				return err
		...
		// 修改策略信息数据
		reqModel := ent.UpgradeConfigurationUpgradeStrategy{
			ID: *req.Id,
		}
		...
		err = l.svcCtx.DB.UpgradeConfigurationUpgradeStrategy.UpdateOne(&reqModel).
...
func (l *UpdateUpgradeConfigurationUpgradeStrategyLogic) CheckUpdateUpgradeConfigurationUpgradeStrategy(req *types.UpgradeConfigurationUpgradeStrategyInfo) error {
	// 判断是否重复
	var predicates []predicate.UpgradeConfigurationUpgradeStrategy
	predicates = append(predicates, upgradeconfigurationupgradestrategy.IDNEQ(*req.Id))
	...
	predicates = append(predicates, upgradeconfigurationupgradestrategy.CompanyIDEQ(companyctx.GetCompanyIDFromCtx(l.ctx)))
