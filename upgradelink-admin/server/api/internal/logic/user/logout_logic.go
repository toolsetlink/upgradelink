...
func (l *LogoutLogic) Logout() (resp *types.BaseMsgResp, err error) {
	...
	err = l.svcCtx.DB.Token.Update().Where(token.UUIDEQ(uuidx.ParseUUIDString(userId))).SetStatus(enum.StatusBanned).Exec(l.ctx)
	if err != nil {
		return nil, db_error.DefaultEntError(l.Logger, err, "")
	}

	tokenData, err := l.svcCtx.DB.Token.Query().
		Where(token.UUIDEQ(uuidx.ParseUUIDString(userId))).
