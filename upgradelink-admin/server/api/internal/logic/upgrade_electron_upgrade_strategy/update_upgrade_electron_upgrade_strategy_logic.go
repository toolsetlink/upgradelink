...
type UpdateUpgradeElectronUpgradeStrategyLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewUpdateUpgradeElectronUpgradeStrategyLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateUpgradeElectronUpgradeStrategyLogic {
	return &UpdateUpgradeElectronUpgradeStrategyLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *UpdateUpgradeElectronUpgradeStrategyLogic) UpdateUpgradeElectronUpgradeStrategy(req *types.UpgradeElectronUpgradeStrategyInfo) (*types.BaseMsgResp, error) {

	// 效验请求参数数据
	err := l.CheckUpdateUpgradeElectronUpgradeStrategy(req)
	if err != nil {
		return nil, err
	}

	// 获取策略信息
	strategyData, err := l.svcCtx.DB.UpgradeElectronUpgradeStrategy.Get(l.ctx, *req.Id)
	if err != nil {
	...
	if err := entx.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		for i := 0; i < len(flowLimitIds); i++ {
			err = l.svcCtx.DB.UpgradeElectronUpgradeStrategyFlowLimitStrategy.Update().
				Where(upgradeelectronupgradestrategyflowlimitstrategy.IDEQ(flowLimitIds[i])).
				SetNotNilIsDel(&intDelTrue).
				Exec(l.ctx)
			if err != nil {
				return err
		...
		if *req.IsGray == 1 {
			for i := 0; i < len(req.GrayDataInfo); i++ {
				...
				grayInfo, err := l.svcCtx.DB.UpgradeElectronUpgradeStrategyGrayStrategy.Create().
		...
		if *req.IsFlowLimit == 1 {
			for i := 0; i < len(req.FlowLimitDataInfo); i++ {
				...
				flowLimitInfo, err := l.svcCtx.DB.UpgradeElectronUpgradeStrategyFlowLimitStrategy.Create().
		...
		// 修改策略信息数据
		reqModel := ent.UpgradeElectronUpgradeStrategy{
			ID: *req.Id,
		}
		...
		err = l.svcCtx.DB.UpgradeElectronUpgradeStrategy.UpdateOne(&reqModel).
			SetNotNilEnable(req.Enable).
			SetNotNilName(req.Name).
			SetNotNilDescription(req.Description).
			SetNotNilElectronID(req.ElectronId).
			SetNotNilElectronVersionID(req.ElectronVersionId).
...
func (l *UpdateUpgradeElectronUpgradeStrategyLogic) CheckUpdateUpgradeElectronUpgradeStrategy(req *types.UpgradeElectronUpgradeStrategyInfo) error {
	// 判断是否重复
	var predicates []predicate.UpgradeElectronUpgradeStrategy
	predicates = append(predicates, upgradeelectronupgradestrategy.IDNEQ(*req.Id))
	predicates = append(predicates, upgradeelectronupgradestrategy.Name(*req.Name))
	...
	count, err := l.svcCtx.DB.UpgradeElectronUpgradeStrategy.Query().Where(predicates...).Count(l.ctx)
	if err != nil {
		return err
	}
	if count > 0 {
