...
type UpdateUpgradeWinUpgradeStrategyLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewUpdateUpgradeWinUpgradeStrategyLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateUpgradeWinUpgradeStrategyLogic {
	return &UpdateUpgradeWinUpgradeStrategyLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *UpdateUpgradeWinUpgradeStrategyLogic) UpdateUpgradeWinUpgradeStrategy(req *types.UpgradeWinUpgradeStrategyInfo) (*types.BaseMsgResp, error) {

	// 效验请求参数数据
	err := l.CheckUpdateUpgradeWinUpgradeStrategy(req)
	if err != nil {
		return nil, err
	}

	// 获取策略信息
	strategyData, err := l.svcCtx.DB.UpgradeWinUpgradeStrategy.Get(l.ctx, *req.Id)
	if err != nil {
	...
	if err := entx.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		for i := 0; i < len(grayIds); i++ {
			err = l.svcCtx.DB.UpgradeWinUpgradeStrategyGrayStrategy.Update().
				Where(upgradewinupgradestrategygraystrategy.IDEQ(grayIds[i])).
		...
		for i := 0; i < len(flowLimitIds); i++ {
			err = l.svcCtx.DB.UpgradeWinUpgradeStrategyFlowLimitStrategy.Update().
				Where(upgradewinupgradestrategyflowlimitstrategy.IDEQ(flowLimitIds[i])).
				SetNotNilIsDel(&intDelTrue).
				Exec(l.ctx)
			if err != nil {
				return err
		...
		if *req.IsGray == 1 {
			for i := 0; i < len(req.GrayDataInfo); i++ {
				...
				grayInfo, err := l.svcCtx.DB.UpgradeWinUpgradeStrategyGrayStrategy.Create().
		...
		if *req.IsFlowLimit == 1 {
			for i := 0; i < len(req.FlowLimitDataInfo); i++ {
				...
				flowLimitInfo, err := l.svcCtx.DB.UpgradeWinUpgradeStrategyFlowLimitStrategy.Create().
		...
		// 修改策略信息数据
		reqModel := ent.UpgradeWinUpgradeStrategy{
			ID: *req.Id,
		}
		...
		err = l.svcCtx.DB.UpgradeWinUpgradeStrategy.UpdateOne(&reqModel).
			SetNotNilEnable(req.Enable).
			SetNotNilName(req.Name).
			SetNotNilDescription(req.Description).
			SetNotNilWinID(req.WinId).
			SetNotNilWinVersionID(req.WinVersionId).
...
func (l *UpdateUpgradeWinUpgradeStrategyLogic) CheckUpdateUpgradeWinUpgradeStrategy(req *types.UpgradeWinUpgradeStrategyInfo) error {
	// 判断是否重复
	var predicates []predicate.UpgradeWinUpgradeStrategy
	predicates = append(predicates, upgradewinupgradestrategy.IDNEQ(*req.Id))
	predicates = append(predicates, upgradewinupgradestrategy.Name(*req.Name))
	...
	count, err := l.svcCtx.DB.UpgradeWinUpgradeStrategy.Query().Where(predicates...).Count(l.ctx)
	if err != nil {
		return err
	}
	if count > 0 {
