...
type DeleteUpgradeWinUpgradeStrategyLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewDeleteUpgradeWinUpgradeStrategyLogic(ctx context.Context, svcCtx *svc.ServiceContext) *DeleteUpgradeWinUpgradeStrategyLogic {
	return &DeleteUpgradeWinUpgradeStrategyLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *DeleteUpgradeWinUpgradeStrategyLogic) DeleteUpgradeWinUpgradeStrategy(req *types.IDsReq) (*types.BaseMsgResp, error) {
	...
	if err := entx.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		for i := 0; i < len(req.Ids); i++ {
			...
			// 获取策略信息
			strategyData, err := l.svcCtx.DB.UpgradeWinUpgradeStrategy.Get(l.ctx, int(req.Ids[i]))
			if err != nil {
				return err
			}
			...
			for i := 0; i < len(flowLimitIds); i++ {
				err = l.svcCtx.DB.UpgradeWinUpgradeStrategy.Update().
					Where(upgradewinupgradestrategy.IDEQ(flowLimitIds[i])).
					SetNotNilIsDel(&intDelTrue).
					Exec(l.ctx)
				if err != nil {
					return err
			...
			err = l.svcCtx.DB.UpgradeWinUpgradeStrategy.Update().
				Where(upgradewinupgradestrategy.IDIn(Ids...)).
				SetNotNilIsDel(&intDelTrue).
				Exec(l.ctx)
			if err != nil {
				return err
