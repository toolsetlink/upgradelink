...
type UpdateUpgradeFileUpgradeStrategyLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewUpdateUpgradeFileUpgradeStrategyLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateUpgradeFileUpgradeStrategyLogic {
	return &UpdateUpgradeFileUpgradeStrategyLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *UpdateUpgradeFileUpgradeStrategyLogic) UpdateUpgradeFileUpgradeStrategy(req *types.UpgradeFileUpgradeStrategyInfo) (*types.BaseMsgResp, error) {

	// 效验请求参数数据
	err := l.CheckUpdateUpgradeFileUpgradeStrategy(req)
	if err != nil {
		return nil, err
	}

	// 获取策略信息
	strategyData, err := l.svcCtx.DB.UpgradeFileUpgradeStrategy.Get(l.ctx, *req.Id)
	if err != nil {
	...
	if err := entx.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		for i := 0; i < len(flowLimitIds); i++ {
			err = l.svcCtx.DB.UpgradeFileUpgradeStrategyFlowLimitStrategy.Update().
				Where(upgradefileupgradestrategyflowlimitstrategy.IDEQ(flowLimitIds[i])).
				SetNotNilIsDel(&intDelTrue).
				Exec(l.ctx)
			if err != nil {
				return err
		...
		// 修改策略信息数据
		reqModel := ent.UpgradeFileUpgradeStrategy{
			ID: *req.Id,
		}
		...
		err = l.svcCtx.DB.UpgradeFileUpgradeStrategy.UpdateOne(&reqModel).
...
func (l *UpdateUpgradeFileUpgradeStrategyLogic) CheckUpdateUpgradeFileUpgradeStrategy(req *types.UpgradeFileUpgradeStrategyInfo) error {
	// 判断是否重复
	var predicates []predicate.UpgradeFileUpgradeStrategy
	predicates = append(predicates, upgradefileupgradestrategy.IDNEQ(*req.Id))
