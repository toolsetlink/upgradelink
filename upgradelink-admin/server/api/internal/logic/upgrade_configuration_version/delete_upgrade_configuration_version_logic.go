...
func (l *DeleteUpgradeConfigurationVersionLogic) DeleteUpgradeConfigurationVersion(req *types.IDsReq) (*types.BaseMsgResp, error) {
	...
	for _, id := range req.Ids {
		...
		// 删除状态
		predicates = append(predicates, upgradeconfigurationupgradestrategy.ConfigurationVersionID(int(id)))
		...
		if len(strategyList) > 0 {
			return nil, http_error.NewCodeBadRequestError("版本还存在对应的策略，不能删除")
		}
	}
	...
	err := l.svcCtx.DB.UpgradeConfigurationVersion.Update().
		Where(upgradeconfigurationversion.IDIn(Ids...), upgradeconfigurationversion.CompanyIDIn(companyctx.GetCompanyIDFromCtx(l.ctx))).
