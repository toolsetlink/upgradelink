...
type UpdateUpgradeMacUpgradeStrategyLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewUpdateUpgradeMacUpgradeStrategyLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateUpgradeMacUpgradeStrategyLogic {
	return &UpdateUpgradeMacUpgradeStrategyLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *UpdateUpgradeMacUpgradeStrategyLogic) UpdateUpgradeMacUpgradeStrategy(req *types.UpgradeMacUpgradeStrategyInfo) (*types.BaseMsgResp, error) {

	// 效验请求参数数据
	err := l.CheckUpdateUpgradeMacUpgradeStrategy(req)
	if err != nil {
		return nil, err
	}

	// 获取策略信息
	strategyData, err := l.svcCtx.DB.UpgradeMacUpgradeStrategy.Get(l.ctx, *req.Id)
	if err != nil {
	...
	if err := entx.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		for i := 0; i < len(flowLimitIds); i++ {
			err = l.svcCtx.DB.UpgradeMacUpgradeStrategyFlowLimitStrategy.Update().
				Where(upgrademacupgradestrategyflowlimitstrategy.IDEQ(flowLimitIds[i])).
				SetNotNilIsDel(&intDelTrue).
				Exec(l.ctx)
			if err != nil {
				return err
		...
		if *req.IsGray == 1 {
			for i := 0; i < len(req.GrayDataInfo); i++ {
				...
				grayInfo, err := l.svcCtx.DB.UpgradeMacUpgradeStrategyGrayStrategy.Create().
		...
		if *req.IsFlowLimit == 1 {
			for i := 0; i < len(req.FlowLimitDataInfo); i++ {
				...
				flowLimitInfo, err := l.svcCtx.DB.UpgradeMacUpgradeStrategyFlowLimitStrategy.Create().
		...
		// 修改策略信息数据
		reqModel := ent.UpgradeMacUpgradeStrategy{
			ID: *req.Id,
		}
		...
		err = l.svcCtx.DB.UpgradeMacUpgradeStrategy.UpdateOne(&reqModel).
			SetNotNilEnable(req.Enable).
			SetNotNilName(req.Name).
			SetNotNilDescription(req.Description).
			SetNotNilMACID(req.MacId).
			SetNotNilMACVersionID(req.MacVersionId).
...
func (l *UpdateUpgradeMacUpgradeStrategyLogic) CheckUpdateUpgradeMacUpgradeStrategy(req *types.UpgradeMacUpgradeStrategyInfo) error {
	// 判断是否重复
	var predicates []predicate.UpgradeMacUpgradeStrategy
	predicates = append(predicates, upgrademacupgradestrategy.IDNEQ(*req.Id))
	predicates = append(predicates, upgrademacupgradestrategy.Name(*req.Name))
	...
	count, err := l.svcCtx.DB.UpgradeMacUpgradeStrategy.Query().Where(predicates...).Count(l.ctx)
	if err != nil {
		return err
	}
	if count > 0 {
