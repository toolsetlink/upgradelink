...
type GetWinUpgradeInfoLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetWinUpgradeInfoLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetWinUpgradeInfoLogic {
	return &GetWinUpgradeInfoLogic{
...
func (l *GetWinUpgradeInfoLogic) GetWinUpgradeInfo(req *types.GetWinUpgradeInfoReq) (resp *types.GetWinUpgradeInfoResp, err error) {
	...
	var res types.GetWinUpgradeInfoResp
	...
	_, err = l.svcCtx.ResourceCtx.GetWinVersionListByWinIdAndArchAndVersionCode(l.ctx, winInfo.Id, req.Arch, req.VersionCode)
	if err != nil && errors.Is(err, model.ErrNotFound) {
		res.Code = 0
	...
	// 推出可使用的最高版本的升级策略
	winStrategyInfo, err := l.ReturnUpgradeStrategyInfo(winInfo.Id, req.Arch, req.VersionCode, req.AppointVersionCode, req.DevModelKey, req.DevKey)
	if err != nil && errors.Is(err, model.ErrNotFound) {
		res.Code = 0
		res.Msg = l.svcCtx.Trans.Trans(l.ctx, "common.alreadyLatestVersion")
		res.Docs = l.svcCtx.Trans.Trans(l.ctx, "common.alreadyLatestVersionDocs")
	...
	flowLimitOk, err := l.CheckUpgradeStrategyFlowLimit(winStrategyInfo)
	if err != nil {
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, l.svcCtx.Trans.Trans(l.ctx, "common.databaseError"), l.svcCtx.Trans.Trans(l.ctx, "common.databaseErrorDocs"))
	}
	if !flowLimitOk {
	...
	winVersionInfo, err := l.svcCtx.ResourceCtx.GetWinVersionInfoById(l.ctx, winStrategyInfo.WinVersionId)
	if err != nil && errors.Is(err, model.ErrNotFound) {
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, l.svcCtx.Trans.Trans(l.ctx, "win.versionNotFound"), l.svcCtx.Trans.Trans(l.ctx, "win.versionNotFoundDocs"))
	} else if err != nil {
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, l.svcCtx.Trans.Trans(l.ctx, "common.databaseError"), l.svcCtx.Trans.Trans(l.ctx, "common.databaseErrorDocs"))
	}
	...
	_, err = l.svcCtx.ResourceCtx.AddAppUpgradeGetStrategyReportLog(l.ctx, resource.AddAppUpgradeGetStrategyReportLogReq{
		...
		AppVersionTarget:    "",
		AppVersionArch:      "",
		StrategyVersionId:   winStrategyInfo.WinVersionId,
		StrategyVersionCode: winVersionInfo.VersionCode,
		StrategyId:          winStrategyInfo.Id,
	})

	res.Code = 200
	res.Msg = l.svcCtx.Trans.Trans(l.ctx, "common.newVersionAvailable")
	res.Data = types.GetWinUpgradeInfoRespData{
		WinKey:               winInfo.Key,
		...
		UpgradeType:          winStrategyInfo.UpgradeType,
		PromptUpgradeContent: winStrategyInfo.PromptUpgradeContent,
	}
	return &res, nil
}
...
// 根据设备信息 获取对应可用的策略
func (l *GetWinUpgradeInfoLogic) ReturnUpgradeStrategyInfo(winId int64, arch string, versionCode int64, appointVersionCode int64, devModelKey, devKey string) (resp *model.UpgradeWinUpgradeStrategy, err error) {

	// 获取到大于请求版本的所有策略信息
	winStrategyList, err := l.svcCtx.ResourceCtx.GetWinStrategyListByWinIdAndArchAndVersion(l.ctx, winId, arch, versionCode)
	if err != nil {
		return nil, err
	}

	// 如果获取不到 则代表当前已经为最新版本
	if len(winStrategyList) == 0 {
		return nil, model.ErrNotFound
	}

	// 判断是否设置了期望版本， 如果设置了期望版本则直接获取期望版本的策略
	if appointVersionCode > 0 {
		for i := 0; i < len(winStrategyList); i++ {

			// 通过 Version id 获取到对应的版本信息
			winVersionInfo, err := l.svcCtx.ResourceCtx.GetWinVersionInfoById(l.ctx, winStrategyList[i].WinVersionId)
			if err != nil {
				return nil, err
			}

			if winVersionInfo.VersionCode == appointVersionCode {
				// 判断期望版本的策略的 设备相关策略是否符合
				strategyOk, err := l.CheckUpgradeStrategy(winStrategyList[i], devModelKey, devKey)
				if err != nil {
					return nil, err
				}
				...
				if strategyOk {
					// 判断是否符合灰度策略
					grayOk, err := l.CheckUpgradeStrategyGray(winStrategyList[i])
					if err != nil {
						return nil, err
					}
					if grayOk {
						return winStrategyList[i], nil
					}
				}
...
// 效验此版本是否符合规则
func (l *GetWinUpgradeInfoLogic) CheckUpgradeStrategy(strategyInfo *model.UpgradeWinUpgradeStrategy, devModelKey, devKey string) (ok bool, err error) {
	// 如果策略设备分组为 0 则代表全部设备
...
func (l *GetWinUpgradeInfoLogic) CheckUpgradeStrategyGray(strategyInfo *model.UpgradeWinUpgradeStrategy) (ok bool, err error) {
	// 通过策略信息判断是否开启了灰度策略
	...
	// 查询出灰度策略信息
	grayStrategyList, err := l.svcCtx.ResourceCtx.GetWinGrayStrategyListByIds(l.ctx, strategyInfo.GrayData)
	if err != nil {
		return false, err
	}
	...
	for i := 0; i < len(grayStrategyList); i++ {
		...
		if common.IsTimeOutside(grayStrategyList[i].BeginDatetime, grayStrategyList[i].EndDatetime) {
			...
			// 判断缓存数据是否符合 使用计数器方法
			increment, err := l.svcCtx.ResourceCtx.WinGrayStrategyIncrement(l.ctx, grayStrategyList[i])
			if err != nil {
				return false, err
			}
			if increment {
...
func (l *GetWinUpgradeInfoLogic) CheckUpgradeStrategyFlowLimit(strategyInfo *model.UpgradeWinUpgradeStrategy) (ok bool, err error) {
	// 通过策略信息判断是否开启了频控策略
	...
	// 查询出频控策略信息
	flowLimitStrategyList, err := l.svcCtx.ResourceCtx.GetWinFlowLimitStrategyListByIds(l.ctx, strategyInfo.FlowLimitData)
	if err != nil {
		return false, err
	}
	...
	for i := 0; i < len(flowLimitStrategyList); i++ {
		...
		if now.Before(start) || now.After(end) {
			...
			// 判断缓存数据是否符合 使用计数器方法
			increment, err := l.svcCtx.ResourceCtx.WinFlowLimitStrategyIncrement(l.ctx, flowLimitStrategyList[i])
			if err != nil {
				return false, err
			}
			if increment {
