...
func (l *GetFileUpgradeInfoLogic) GetFileUpgradeInfo(req *types.GetFileUpgradeInfoReq) (resp *types.GetFileUpgradeInfoResp, err error) {
	...
	var res types.GetFileUpgradeInfoResp
	...
	_, err = l.svcCtx.ResourceCtx.GetFileVersionListByFileIdAndVersionCode(l.ctx, fileInfo.Id, req.VersionCode)
	if err != nil && errors.Is(err, model.ErrNotFound) {
		res.Code = 0
		res.Msg = "已经是最新版本"
		return &res, nil
	...
	// 推出可使用的最高版本的升级策略
	fileStrategyInfo, err := l.ReturnUpgradeStrategyInfo(fileInfo.Id, req.VersionCode, req.AppointVersionCode, req.DevModelKey, req.DevKey)
	if err != nil && errors.Is(err, model.ErrNotFound) {
		res.Code = 0
		res.Msg = "已经是最新版本"
		return &res, nil
	...
	flowLimitOk, err := l.CheckUpgradeStrategyFlowLimit(fileStrategyInfo)
	if err != nil {
		return nil, err
	}
	if !flowLimitOk {
	...
	fileVersionInfo, err := l.svcCtx.ResourceCtx.GetFileVersionInfoById(l.ctx, fileStrategyInfo.FileVersionId)
	if err != nil && errors.Is(err, model.ErrNotFound) {
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, config.Err1Msg, config.Err1Docs)
	} else if err != nil {
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, config.Err1Msg, config.Err1Docs)
	...
	_, err = l.svcCtx.ResourceCtx.AddAppUpgradeGetStrategyReportLog(l.ctx, resource.AddAppUpgradeGetStrategyReportLogReq{
		...
		AppVersionTarget:    "",
		AppVersionArch:      "",
		StrategyVersionId:   fileStrategyInfo.FileVersionId,
		StrategyVersionCode: fileVersionInfo.VersionCode,
		StrategyId:          fileStrategyInfo.Id,
	})

	res.Code = 200
	res.Msg = "获取到新版本"
	res.Data = types.GetFileUpgradeInfoRespData{
		FileKey:              fileInfo.Key,
		VersionName:          fileVersionInfo.VersionName,
		VersionCode:          fileVersionInfo.VersionCode,
		UrlPath:              urlPath,
		UrlFileSize:          cloudFileInfo.Size,
		UpgradeType:          fileStrategyInfo.UpgradeType,
		PromptUpgradeContent: fileStrategyInfo.PromptUpgradeContent,
	}
	return &res, nil
}
...
// 根据设备信息 获取对应可用的策略
func (l *GetFileUpgradeInfoLogic) ReturnUpgradeStrategyInfo(fileId int64, versionCode int64, appointVersionCode int64, devModelKey, devKey string) (resp *model.UpgradeFileUpgradeStrategy, err error) {

	// 获取到大于请求版本的所有策略信息
	fileStrategyList, err := l.svcCtx.ResourceCtx.GetFileStrategyListByFileIdAndVersion(l.ctx, fileId, versionCode)
	if err != nil {
		return nil, err
	}

	// 如果获取不到 则代表当前已经为最新版本
	if len(fileStrategyList) == 0 {
		return nil, model.ErrNotFound
	}

	// 判断是否设置了期望版本， 如果设置了期望版本则直接获取期望版本的策略
	if appointVersionCode > 0 {
		for i := 0; i < len(fileStrategyList); i++ {

			// 通过 Version id 获取到对应的版本信息
			fileVersionInfo, err := l.svcCtx.ResourceCtx.GetFileVersionInfoById(l.ctx, fileStrategyList[i].FileVersionId)
			if err != nil {
				return nil, err
			}

			if fileVersionInfo.VersionCode == appointVersionCode {
				// 判断期望版本的策略的 设备相关策略是否符合
				strategyOk, err := l.CheckUpgradeStrategy(fileStrategyList[i], devModelKey, devKey)
				if err != nil {
					return nil, err
				}
				...
				if strategyOk {
					// 判断是否符合灰度策略
					grayOk, err := l.CheckUpgradeStrategyGray(fileStrategyList[i])
					if err != nil {
						return nil, err
					}
					if grayOk {
						return fileStrategyList[i], nil
					}
				}
...
// 效验此版本是否符合规则
func (l *GetFileUpgradeInfoLogic) CheckUpgradeStrategy(strategyInfo *model.UpgradeFileUpgradeStrategy, devModelKey, devKey string) (ok bool, err error) {
	// 如果策略设备分组为 0 则代表全部设备
...
func (l *GetFileUpgradeInfoLogic) CheckUpgradeStrategyGray(strategyInfo *model.UpgradeFileUpgradeStrategy) (ok bool, err error) {
	// 通过策略信息判断是否开启了灰度策略
	...
	// 查询出灰度策略信息
	grayStrategyList, err := l.svcCtx.ResourceCtx.GetFileGrayStrategyListByIds(l.ctx, strategyInfo.GrayData)
	if err != nil {
		return false, err
	}
	...
	for i := 0; i < len(grayStrategyList); i++ {
		...
		if common.IsTimeOutside(grayStrategyList[i].BeginDatetime, grayStrategyList[i].EndDatetime) {
			...
			// 判断缓存数据是否符合 使用计数器方法
			increment, err := l.svcCtx.ResourceCtx.FileGrayStrategyIncrement(l.ctx, grayStrategyList[i])
			if err != nil {
				return false, err
			}
			if increment {
...
func (l *GetFileUpgradeInfoLogic) CheckUpgradeStrategyFlowLimit(strategyInfo *model.UpgradeFileUpgradeStrategy) (ok bool, err error) {
	// 通过策略信息判断是否开启了频控策略
	...
	// 查询出频控策略信息
	flowLimitStrategyList, err := l.svcCtx.ResourceCtx.GetFileFlowLimitStrategyListByIds(l.ctx, strategyInfo.FlowLimitData)
	if err != nil {
		return false, err
	}
	...
	for i := 0; i < len(flowLimitStrategyList); i++ {
		...
		if now.Before(start) || now.After(end) {
			...
			// 判断缓存数据是否符合 使用计数器方法
			increment, err := l.svcCtx.ResourceCtx.FileFlowLimitStrategyIncrement(l.ctx, flowLimitStrategyList[i])
			if err != nil {
				return false, err
			}
			if increment {
