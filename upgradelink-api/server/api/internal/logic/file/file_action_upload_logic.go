...
type FileActionUploadLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewFileActionUploadLogic(ctx context.Context, svcCtx *svc.ServiceContext) *FileActionUploadLogic {
	return &FileActionUploadLogic{
...
// 5. 创建文件升级策略
// 6. 返回上传结果
func (l *FileActionUploadLogic) FileActionUpload(req *types.FileActionUploadReq) (resp *types.FileActionUploadResp, err error) {
	// 1. 验证应用信息
	fileInfo, err := l.validateFileInfo(req)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		...
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, l.svcCtx.Trans.Trans(l.ctx, "common.downloadFailed"), "")
	}
	...
	if err != nil {
		...
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, l.svcCtx.Trans.Trans(l.ctx, "common.uploadFailed"), "")
	}

	// 4. 创建文件版本
	versionId, err := l.createFileVersion(fileInfo, fileUUID, req.Version)
	if err != nil {
		l.Errorf("创建文件版本失败: %w", err)
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, l.svcCtx.Trans.Trans(l.ctx, "common.createFailed"), "")
	}

	// 5. 创建文件升级策略
	err = l.createFileUpgradeStrategy(fileInfo, versionId, req.Version, req.PromptUpgradeContent)
	if err != nil {
		l.Errorf("创建文件升级策略失败: %w", err)
		return nil, http_handlers.NewLinkErr(l.ctx, http_handlers.ErrInternalServerError, l.svcCtx.Trans.Trans(l.ctx, "common.createFailed"), "")
	}

	// 6. 返回上传结果
	return &types.FileActionUploadResp{
...
// validateFileInfo 验证文件信息
func (l *FileActionUploadLogic) validateFileInfo(req *types.FileActionUploadReq) (*model.UpgradeFile, error) {
	// 通过 key 获取文件信息
	fileInfo, err := l.svcCtx.ResourceCtx.GetFileInfoByKey(l.ctx, req.AppKey)
	if err != nil {
...
// downloadFile 下载文件
func (l *FileActionUploadLogic) downloadFile(url, outputDir string) (string, string, error) {
	// 从 URL 中提取文件名
...
func (l *FileActionUploadLogic) uploadFileToOss(filePath, fileGithubPath string) (string, error) {
...
func (l *FileActionUploadLogic) getFileSuffix(fileName string) (string, string, error) {
	// 校验文件后缀
...
func (l *FileActionUploadLogic) getFileType(fileName string) string {
...
func (l *FileActionUploadLogic) createFileVersion(fileInfo *model.UpgradeFile, fileUUID, version string) (int64, error) {
	// 转换版本号为数字格式
	...
	// 创建文件版本
	versionId, err := l.svcCtx.ResourceCtx.AddFileVersion(l.ctx, resource.AddFileVersionReq{
	...
	if err != nil {
		l.Errorf("创建文件版本失败: %w", err)
		return 0, fmt.Errorf("创建文件版本失败: %w", err)
	}

	return versionId, nil
}

// createFileUpgradeStrategy 创建文件升级策略
func (l *FileActionUploadLogic) createFileUpgradeStrategy(fileInfo *model.UpgradeFile, versionId int64, version, promptUpgradeContent string) error {
	// 使用时区统一时间点
	now := time.Now().Local()
	end := now.AddDate(1, 0, 0)

	// 创建文件升级策略
	_, err := l.svcCtx.ResourceCtx.AddFileStrategy(l.ctx, resource.AddFileStrategyReq{
		CompanyId:            fileInfo.CompanyId,
		Enable:               1,
		Name:                 fmt.Sprintf("%s-%s", fileInfo.Name, version),
	...
	if err != nil {
		l.Errorf("创建文件升级策略失败: %w", err)
		return fmt.Errorf("创建文件升级策略失败: %w", err)
	}
