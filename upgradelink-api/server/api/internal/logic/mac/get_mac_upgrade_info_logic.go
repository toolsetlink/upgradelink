...
type GetMacUpgradeInfoLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetMacUpgradeInfoLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetMacUpgradeInfoLogic {
	return &GetMacUpgradeInfoLogic{
...
func (l *GetMacUpgradeInfoLogic) GetMacUpgradeInfo(req *types.GetMacUpgradeInfoReq) (resp *types.GetMacUpgradeInfoResp, err error) {
	...
	flowLimitOk, err := l.CheckUpgradeStrategyFlowLimit(macStrategyInfo)
	if err != nil {
		return nil, err
	}
	if !flowLimitOk {
	...
	_, err = l.svcCtx.ResourceCtx.AddAppUpgradeGetStrategyReportLog(l.ctx, resource.AddAppUpgradeGetStrategyReportLogReq{
		...
		AppVersionTarget:    "",
		AppVersionArch:      "",
		StrategyVersionId:   macStrategyInfo.MacVersionId,
		StrategyVersionCode: macVersionInfo.VersionCode,
		StrategyId:          macStrategyInfo.Id,
	})
	...
	res.Data = types.GetMacUpgradeInfoRespData{
		...
		UpgradeType:          macStrategyInfo.UpgradeType,
		PromptUpgradeContent: macStrategyInfo.PromptUpgradeContent,
	}
	return &res, nil
}
...
// 根据设备信息 获取对应可用的策略
func (l *GetMacUpgradeInfoLogic) ReturnUpgradeStrategyInfo(macId int64, arch string, versionCode int64, appointVersionCode int64, devModelKey, devKey string) (resp *model.UpgradeMacUpgradeStrategy, err error) {

	// 获取到大于请求版本的所有策略信息
	macStrategyList, err := l.svcCtx.ResourceCtx.GetMacStrategyListByMacIdAndArchAndVersion(l.ctx, macId, arch, versionCode)
	if err != nil {
		return nil, err
	}

	// 如果获取不到 则代表当前已经为最新版本
	if len(macStrategyList) == 0 {
		return nil, model.ErrNotFound
	}

	// 判断是否设置了期望版本， 如果设置了期望版本则直接获取期望版本的策略
	if appointVersionCode > 0 {
		for i := 0; i < len(macStrategyList); i++ {

			// 通过 Version id 获取到对应的版本信息
			macVersionInfo, err := l.svcCtx.ResourceCtx.GetMacVersionInfoById(l.ctx, macStrategyList[i].MacVersionId)
			if err != nil {
				return nil, err
			}

			if macVersionInfo.VersionCode == appointVersionCode {
				// 判断期望版本的策略的 设备相关策略是否符合
				strategyOk, err := l.CheckUpgradeStrategy(macStrategyList[i], devModelKey, devKey)
				if err != nil {
					return nil, err
				}
				...
				if strategyOk {
					// 判断是否符合灰度策略
					grayOk, err := l.CheckUpgradeStrategyGray(macStrategyList[i])
					if err != nil {
						return nil, err
					}
					if grayOk {
						return macStrategyList[i], nil
					}
				}
...
// 效验此版本是否符合规则
func (l *GetMacUpgradeInfoLogic) CheckUpgradeStrategy(strategyInfo *model.UpgradeMacUpgradeStrategy, devModelKey, devKey string) (ok bool, err error) {
	// 如果策略设备分组为 0 则代表全部设备
...
func (l *GetMacUpgradeInfoLogic) CheckUpgradeStrategyGray(strategyInfo *model.UpgradeMacUpgradeStrategy) (ok bool, err error) {
	// 通过策略信息判断是否开启了灰度策略
	...
	// 查询出灰度策略信息
	grayStrategyList, err := l.svcCtx.ResourceCtx.GetMacGrayStrategyListByIds(l.ctx, strategyInfo.GrayData)
	if err != nil {
		return false, err
	}
	...
	for i := 0; i < len(grayStrategyList); i++ {
		...
		if common.IsTimeOutside(grayStrategyList[i].BeginDatetime, grayStrategyList[i].EndDatetime) {
			...
			// 判断缓存数据是否符合 使用计数器方法
			increment, err := l.svcCtx.ResourceCtx.MacGrayStrategyIncrement(l.ctx, grayStrategyList[i])
			if err != nil {
				return false, err
			}
			if increment {
...
func (l *GetMacUpgradeInfoLogic) CheckUpgradeStrategyFlowLimit(strategyInfo *model.UpgradeMacUpgradeStrategy) (ok bool, err error) {
	// 通过策略信息判断是否开启了频控策略
	...
	// 查询出频控策略信息
	flowLimitStrategyList, err := l.svcCtx.ResourceCtx.GetMacFlowLimitStrategyListByIds(l.ctx, strategyInfo.FlowLimitData)
	if err != nil {
		return false, err
	}
	...
	for i := 0; i < len(flowLimitStrategyList); i++ {
		...
		if now.Before(start) || now.After(end) {
			...
			// 判断缓存数据是否符合 使用计数器方法
			increment, err := l.svcCtx.ResourceCtx.MacFlowLimitStrategyIncrement(l.ctx, flowLimitStrategyList[i])
			if err != nil {
				return false, err
			}
			if increment {
