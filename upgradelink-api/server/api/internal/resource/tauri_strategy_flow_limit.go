...
const (
	CacheKeyGetTauriFlowLimitStrategyListByIds              = PREFIX + "TAURI_FLOW_LIMIT_LIST:IDS:%v"
	CacheKeyGetTauriFlowLimitStrategyIncrementByIdAndPeriod = PREFIX + "TAURI_FLOW_LIMIT_STRATEGY_INCREMENT:ID:%v:PERIOD:%v"
)

// GetTauriFlowLimitStrategyListByIds
// 通过 ids 获取到流控策略list
func (c *Ctx) GetTauriFlowLimitStrategyListByIds(ctx context.Context, ids string) ([]*model.UpgradeTauriUpgradeStrategyFlowLimitStrategy, error) {

	cacheKey := fmt.Sprintf(CacheKeyGetTauriFlowLimitStrategyListByIds, ids)
	// 内存缓存
	v, err := c.localCache.Take(cacheKey, func() (interface{}, error) {
		...
		// 构建 SQL 查询语句
		query := fmt.Sprintf("select * from upgrade_tauri_upgrade_strategy_flow_limit_strategy "+
			"where id in ( %s ) "+
			"AND enable = 1 "+
		...
		// sql 缓存查询
		var list []*model.UpgradeTauriUpgradeStrategyFlowLimitStrategy
	...
	if v != nil {
		list := v.([]*model.UpgradeTauriUpgradeStrategyFlowLimitStrategy)
		return list, err
	}
...
// TauriFlowLimitStrategyIncrement 判断当前请求在流控策略要求的数量范围内  使用 redis 计数器方式实现
func (c *Ctx) TauriFlowLimitStrategyIncrement(ctx context.Context, flowLimitStrategy *model.UpgradeTauriUpgradeStrategyFlowLimitStrategy) (bool, error) {
	...
	// 构建 Redis 键名
	cacheKey := fmt.Sprintf(CacheKeyGetTauriFlowLimitStrategyIncrementByIdAndPeriod, flowLimitStrategy.Id, period)
