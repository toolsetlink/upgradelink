...
export function useModalDraggable(targetRef, dragRef, draggable, containerSelector, centered) {
  ...
  const onMousedown = (e) => {
    ...
    const onMousemove = (e2) => {
      ...
      if (targetRef.value) {
        ...
        targetRef.value.style.transform = isCentered ? `translate(${moveX}px, calc(-50% + ${moveY}px))` : `translate(${moveX}px, ${moveY}px)`;
        dragging.value = true;
      }
    };
    const onMouseup = () => {
