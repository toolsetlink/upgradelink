...
import type { UpgradeLnxInfo } from "#/api/upgrade/model/upgradeLnxModel";
...
import { deleteUpgradeLnx, getUpgradeLnxList } from "#/api/upgrade/upgradeLnx";
import { TableAction } from "#/components/table/table-action";

import UpgradeLnxForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeLnxManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeLnxForm,
});
...
const gridOptions: VxeGridProps<UpgradeLnxInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeLnx({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t("upgrade.upgradeLnx.addUpgradeLnx") }}
        </Button>
      </template>
    </Grid>
  </Page>
