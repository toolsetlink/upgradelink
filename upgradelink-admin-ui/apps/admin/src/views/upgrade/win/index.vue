...
import type { UpgradeWinInfo } from "#/api/upgrade/model/upgradeWinModel";
...
import { deleteUpgradeWin, getUpgradeWinList } from "#/api/upgrade/upgradeWin";
import { TableAction } from "#/components/table/table-action";

import UpgradeWinForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeWinManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeWinForm,
});
...
const gridOptions: VxeGridProps<UpgradeWinInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeWin({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t("upgrade.upgradeWin.addUpgradeWin") }}
        </Button>
      </template>
    </Grid>
  </Page>
