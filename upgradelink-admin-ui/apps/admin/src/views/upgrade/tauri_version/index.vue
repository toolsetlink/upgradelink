...
import {
  deleteUpgradeTauriVersion,
  getUpgradeTauriVersionList,
} from "#/api/upgrade/upgradeTauriVersion";
import { TableAction } from "#/components/table/table-action";

import UpgradeTauriVersionForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeTauriVersionManagement",
});
...
const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeTauriVersionForm,
});
...
const gridOptions: VxeGridProps<UpgradeTauriVersionInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeTauriVersion({
    ids,
  });
  if (result.code === 0) {
