...
import { deleteUpgradeMac, getUpgradeMacList } from "#/api/upgrade/upgradeMac";
import { TableAction } from "#/components/table/table-action";

import UpgradeMacForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeMacManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeMacForm,
});
...
const gridOptions: VxeGridProps<UpgradeMacInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeMac({
    ids,
  });
  if (result.code === 0) {
