...
import {
  deleteUpgradeMacUpgradeStrategy,
  getUpgradeMacUpgradeStrategyList,
} from "#/api/upgrade/upgradeMacUpgradeStrategy";
import { TableAction } from "#/components/table/table-action";

import UpgradeMacUpgradeStrategyForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeMacUpgradeStrategyManagement",
});

// ---------------- form -----------------

const [FormDrawer, formDrawerApi] = useVbenDrawer({
  connectedComponent: UpgradeMacUpgradeStrategyForm,
});
...
const gridOptions: VxeGridProps<UpgradeMacUpgradeStrategyInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeMacUpgradeStrategyList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeMacUpgradeStrategy({
    ids,
  });
  if (result.code === 0) {
