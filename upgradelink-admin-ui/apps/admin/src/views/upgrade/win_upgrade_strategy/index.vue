...
import {
  deleteUpgradeWinUpgradeStrategy,
  getUpgradeWinUpgradeStrategyList,
} from "#/api/upgrade/upgradeWinUpgradeStrategy";
import { TableAction } from "#/components/table/table-action";

import UpgradeWinUpgradeStrategyForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeWinUpgradeStrategyManagement",
});

// ---------------- form -----------------

const [FormDrawer, formDrawerApi] = useVbenDrawer({
  connectedComponent: UpgradeWinUpgradeStrategyForm,
});
...
const gridOptions: VxeGridProps<UpgradeWinUpgradeStrategyInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeWinUpgradeStrategyList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeWinUpgradeStrategy({
    ids,
  });
  if (result.code === 0) {
