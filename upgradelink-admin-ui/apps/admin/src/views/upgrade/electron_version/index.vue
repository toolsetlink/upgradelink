...
import {
  deleteUpgradeElectronVersion,
  getUpgradeElectronVersionList,
} from "#/api/upgrade/upgradeElectronVersion";
import { TableAction } from "#/components/table/table-action";

import UpgradeElectronVersionForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeElectronVersionManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeElectronVersionForm,
});
...
const gridOptions: VxeGridProps<UpgradeElectronVersionInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeElectronVersion({
    ids,
  });
  if (result.code === 0) {
...
async function handleDownload(record: any) {
  ...
  link.href = record.installCloudFilePath;
...
function showQrCode(record: any) {
  currentQrCodeUrl.value = record.installCloudFilePath;
