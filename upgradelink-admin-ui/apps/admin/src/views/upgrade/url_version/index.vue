...
import {
  deleteUpgradeUrlVersion,
  getUpgradeUrlVersionList,
} from "#/api/upgrade/upgradeUrlVersion";
import { TableAction } from "#/components/table/table-action";

import UpgradeUrlVersionForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeUrlVersionManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeUrlVersionForm,
});
...
const gridOptions: VxeGridProps<UpgradeUrlVersionInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeUrlVersion({
    ids,
  });
  if (result.code === 0) {
...
function showQrCode(record: any) {
  currentQrCodeUrl.value = record.urlPath;
