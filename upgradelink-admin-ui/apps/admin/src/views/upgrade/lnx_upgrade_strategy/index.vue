...
import {
  deleteUpgradeLnxUpgradeStrategy,
  getUpgradeLnxUpgradeStrategyList,
} from "#/api/upgrade/upgradeLnxUpgradeStrategy";
import { TableAction } from "#/components/table/table-action";

import UpgradeLnxUpgradeStrategyForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeLnxUpgradeStrategyManagement",
});

// ---------------- form -----------------

const [FormDrawer, formDrawerApi] = useVbenDrawer({
  connectedComponent: UpgradeLnxUpgradeStrategyForm,
});
...
const gridOptions: VxeGridProps<UpgradeLnxUpgradeStrategyInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeLnxUpgradeStrategyList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeLnxUpgradeStrategy({
    ids,
  });
  if (result.code === 0) {
