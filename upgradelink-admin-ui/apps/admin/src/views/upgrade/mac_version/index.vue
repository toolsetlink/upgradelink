...
import {
  deleteUpgradeMacVersion,
  getUpgradeMacVersionList,
} from "#/api/upgrade/upgradeMacVersion";
import { TableAction } from "#/components/table/table-action";

import UpgradeMacVersionForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeMacVersionManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeMacVersionForm,
});
...
const gridOptions: VxeGridProps<UpgradeMacVersionInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeMacVersion({
    ids,
  });
  if (result.code === 0) {
