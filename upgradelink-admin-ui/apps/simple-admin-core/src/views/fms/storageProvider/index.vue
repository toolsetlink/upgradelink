...
const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: StorageProviderForm,
});
...
const gridOptions: VxeGridProps<StorageProviderInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getStorageProviderList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteStorageProvider({
    ids,
  });
  if (result.code === 0) {
