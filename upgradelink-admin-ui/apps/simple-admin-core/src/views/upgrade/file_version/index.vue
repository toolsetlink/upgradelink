...
import type { UpgradeFileVersionInfo } from "#/api/upgrade/model/upgradeFileVersionModel";
...
import {
  deleteUpgradeFileVersion,
  getUpgradeFileVersionList,
} from "#/api/upgrade/upgradeFileVersion";
import { TableAction } from "#/components/table/table-action";

import UpgradeFileVersionForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeFileVersionManagement",
});
...
const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeFileVersionForm,
});
...
const gridOptions: VxeGridProps<UpgradeFileVersionInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeFileVersion({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t("upgrade.upgradeFileVersion.addUpgradeFileVersion") }}
        </Button>
      </template>
    </Grid>
  </Page>
