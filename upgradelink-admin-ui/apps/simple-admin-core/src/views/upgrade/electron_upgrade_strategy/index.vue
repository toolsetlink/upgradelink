...
import {
  deleteUpgradeElectronUpgradeStrategy,
  getUpgradeElectronUpgradeStrategyList,
} from "#/api/upgrade/upgradeElectronUpgradeStrategy";
import { TableAction } from "#/components/table/table-action";

import UpgradeElectronUpgradeStrategyForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeElectronUpgradeStrategyManagement",
});

// ---------------- form -----------------

const [FormDrawer, formDrawerApi] = useVbenDrawer({
  connectedComponent: UpgradeElectronUpgradeStrategyForm,
});
...
const gridOptions: VxeGridProps<UpgradeElectronUpgradeStrategyInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeElectronUpgradeStrategyList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeElectronUpgradeStrategy({
    ids,
  });
  if (result.code === 0) {
