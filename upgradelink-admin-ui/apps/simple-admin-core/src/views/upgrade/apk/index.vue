...
import type { UpgradeApkInfo } from "#/api/upgrade/model/upgradeApkModel";
...
import { deleteUpgradeApk, getUpgradeApkList } from "#/api/upgrade/upgradeApk";
import { TableAction } from "#/components/table/table-action";

import UpgradeApkForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeApkManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeApkForm,
});
...
const gridOptions: VxeGridProps<UpgradeApkInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeApk({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t("upgrade.upgradeApk.addUpgradeApk") }}
        </Button>
      </template>
    </Grid>
  </Page>
