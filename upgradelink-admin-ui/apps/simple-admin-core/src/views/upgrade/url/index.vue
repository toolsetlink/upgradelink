...
import type { UpgradeUrlInfo } from "#/api/upgrade/model/upgradeUrlModel";
...
import { deleteUpgradeUrl, getUpgradeUrlList } from "#/api/upgrade/upgradeUrl";
import { TableAction } from "#/components/table/table-action";

import UpgradeUrlForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeUrlManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeUrlForm,
});
...
const gridOptions: VxeGridProps<UpgradeUrlInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeUrl({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t("upgrade.upgradeUrl.addUpgradeUrl") }}
        </Button>
      </template>
    </Grid>
  </Page>
