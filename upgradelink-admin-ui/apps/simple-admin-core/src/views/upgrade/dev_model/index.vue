...
import {
  deleteUpgradeDevModel,
  getUpgradeDevModelList,
} from "#/api/upgrade/upgradeDevModel";
import { TableAction } from "#/components/table/table-action";

import UpgradeDevModelForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeDevModelManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeDevModelForm,
});
...
const gridOptions: VxeGridProps<UpgradeDevModelInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeDevModel({
    ids,
  });
  if (result.code === 0) {
