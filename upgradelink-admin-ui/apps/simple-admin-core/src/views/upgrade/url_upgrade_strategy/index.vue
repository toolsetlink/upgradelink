...
import {
  deleteUpgradeUrlUpgradeStrategy,
  getUpgradeUrlUpgradeStrategyList,
} from "#/api/upgrade/upgradeUrlUpgradeStrategy";
import { TableAction } from "#/components/table/table-action";

import UpgradeUrlUpgradeStrategyForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeUrlUpgradeStrategyManagement",
});

// ---------------- form -----------------

const [FormDrawer, formDrawerApi] = useVbenDrawer({
  connectedComponent: UpgradeUrlUpgradeStrategyForm,
});
...
const gridOptions: VxeGridProps<UpgradeUrlUpgradeStrategyInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeUrlUpgradeStrategyList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeUrlUpgradeStrategy({
    ids,
  });
  if (result.code === 0) {
