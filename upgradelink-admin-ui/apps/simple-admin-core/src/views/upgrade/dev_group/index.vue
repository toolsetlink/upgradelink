...
import type { UpgradeDevGroupInfo } from "#/api/upgrade/model/upgradeDevGroupModel";
...
import {
  deleteUpgradeDevGroup,
  getUpgradeDevGroupList,
} from "#/api/upgrade/upgradeDevGroup";
import { TableAction } from "#/components/table/table-action";

import UpgradeDevGroupForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeDevGroupManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeDevGroupForm,
});
...
const gridOptions: VxeGridProps<UpgradeDevGroupInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeDevGroup({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t("upgrade.upgradeDevGroup.addUpgradeDevGroup") }}
        </Button>
      </template>
    </Grid>
  </Page>
