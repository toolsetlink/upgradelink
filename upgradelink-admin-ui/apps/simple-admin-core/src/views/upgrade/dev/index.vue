...
import { deleteUpgradeDev, getUpgradeDevList } from "#/api/upgrade/upgradeDev";
import { TableAction } from "#/components/table/table-action";

import UpgradeDevForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeDevManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeDevForm,
});
...
const gridOptions: VxeGridProps<UpgradeDevInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeDevList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeDev({
    ids,
  });
  if (result.code === 0) {
