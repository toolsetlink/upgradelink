...
import {
  deleteUpgradeConfiguration,
  getUpgradeConfigurationList,
} from "#/api/upgrade/upgradeConfiguration";
import { TableAction } from "#/components/table/table-action";

import UpgradeConfigurationForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeConfigurationManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeConfigurationForm,
});
...
const gridOptions: VxeGridProps<UpgradeConfigurationInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeConfiguration({
    ids,
  });
  if (result.code === 0) {
