...
import type { UpgradeElectronInfo } from "#/api/upgrade/model/upgradeElectronModel";
...
import {
  deleteUpgradeElectron,
  getUpgradeElectronList,
} from "#/api/upgrade/upgradeElectron";
import { TableAction } from "#/components/table/table-action";

import UpgradeElectronForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeElectronManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeElectronForm,
});
...
const gridOptions: VxeGridProps<UpgradeElectronInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeElectron({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t("upgrade.upgradeElectron.addUpgradeElectron") }}
        </Button>
      </template>
    </Grid>
  </Page>
