...
import {
  deleteUpgradeApkUpgradeStrategy,
  getUpgradeApkUpgradeStrategyList,
} from "#/api/upgrade/upgradeApkUpgradeStrategy";
import { TableAction } from "#/components/table/table-action";

import UpgradeApkUpgradeStrategyForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeApkUpgradeStrategyManagement",
});

// ---------------- form -----------------

const [FormDrawer, formDrawerApi] = useVbenDrawer({
  connectedComponent: UpgradeApkUpgradeStrategyForm,
});
...
const gridOptions: VxeGridProps<UpgradeApkUpgradeStrategyInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeApkUpgradeStrategyList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeApkUpgradeStrategy({
    ids,
  });
  if (result.code === 0) {
