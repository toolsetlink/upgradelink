...
import {
  deleteUpgradeConfigurationVersion,
  getUpgradeConfigurationVersionList,
} from "#/api/upgrade/upgradeConfigurationVersion";
import { TableAction } from "#/components/table/table-action";

import UpgradeConfigurationVersionForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeConfigurationVersionManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeConfigurationVersionForm,
});
...
const gridOptions: VxeGridProps<UpgradeConfigurationVersionInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeConfigurationVersion({
    ids,
  });
  if (result.code === 0) {
