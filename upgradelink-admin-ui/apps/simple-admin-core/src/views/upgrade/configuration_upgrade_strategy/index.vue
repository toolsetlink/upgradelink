...
import type { UpgradeConfigurationUpgradeStrategyInfo } from "#/api/upgrade/model/upgradeConfigurationUpgradeStrategyModel";
...
import {
  deleteUpgradeConfigurationUpgradeStrategy,
  getUpgradeConfigurationUpgradeStrategyList,
} from "#/api/upgrade/upgradeConfigurationUpgradeStrategy";
import { TableAction } from "#/components/table/table-action";

import UpgradeConfigurationUpgradeStrategyForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeConfigurationUpgradeStrategyManagement",
});
...
const gridOptions: VxeGridProps<UpgradeConfigurationUpgradeStrategyInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeConfigurationUpgradeStrategyList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeConfigurationUpgradeStrategy({
    ids,
  });
  if (result.code === 0) {
