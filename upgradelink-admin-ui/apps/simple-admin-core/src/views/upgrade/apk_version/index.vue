...
import type { UpgradeApkVersionInfo } from "#/api/upgrade/model/upgradeApkVersionModel";
...
import {
  deleteUpgradeApkVersion,
  getUpgradeApkVersionList,
} from "#/api/upgrade/upgradeApkVersion";
import { TableAction } from "#/components/table/table-action";

import UpgradeApkVersionForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeApkVersionManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeApkVersionForm,
});
...
const gridOptions: VxeGridProps<UpgradeApkVersionInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeApkVersion({
    ids,
  });
  if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t("upgrade.upgradeApkVersion.addUpgradeApkVersion") }}
        </Button>
      </template>
    </Grid>
  </Page>
