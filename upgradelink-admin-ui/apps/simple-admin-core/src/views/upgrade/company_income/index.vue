...
import {
  deleteUpgradeCompanyIncome,
  getUpgradeCompanyIncomeList,
} from "#/api/upgrade/upgradeCompanyIncome";

import UpgradeCompanyIncomeForm from "./form.vue";
import { searchFormSchemas, tableColumns } from "./schemas";

defineOptions({
  name: "UpgradeCompanyIncomeManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: UpgradeCompanyIncomeForm,
});
...
const gridOptions: VxeGridProps<UpgradeCompanyIncomeInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUpgradeCompanyIncomeList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteUpgradeCompanyIncome({
    ids,
  });
  if (result.code === 0) {
