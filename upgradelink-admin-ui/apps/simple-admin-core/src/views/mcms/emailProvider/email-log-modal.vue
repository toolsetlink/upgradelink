...
import { emailLogSearchFormSchemas, emailLogTableColumns } from "./schemas";
...
const formOptions: VbenFormProps = {
  // 默认展开
  collapsed: false,
  schema: [...(emailLogSearchFormSchemas.schema as any)],
  // 控制表单是否显示折叠按钮
...
const gridOptions: VxeGridProps<DictionaryDetailInfo> = {
  ...
  showOverflow: true,
  columns: [
    ...(emailLogTableColumns.columns as any),
    {
...
async function batchDelete(ids: any[]) {
  const result = await deleteEmailLog({
    ids,
  });
  if (result.code === 0) {
