...
defineOptions({
  name: "CompanySecretManagement",
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: CompanySecretForm,
});
...
const gridOptions: VxeGridProps<CompanySecretInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getCompanySecretList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteCompanySecret({
    ids,
  });
  if (result.code === 0) {
