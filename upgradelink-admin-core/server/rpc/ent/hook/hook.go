...
// The CompanySecretFunc type is an adapter to allow the use of ordinary
// function as CompanySecret mutator.
type CompanySecretFunc func(context.Context, *ent.CompanySecretMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CompanySecretFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CompanySecretMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CompanySecretMutation", m)
}
