...
type UpdateUpgradeApkUpgradeStrategyLogic struct {
	ctx    context.Context
...
func NewUpdateUpgradeApkUpgradeStrategyLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateUpgradeApkUpgradeStrategyLogic {
	return &UpdateUpgradeApkUpgradeStrategyLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *UpdateUpgradeApkUpgradeStrategyLogic) UpdateUpgradeApkUpgradeStrategy(req *types.UpgradeApkUpgradeStrategyInfo) (*types.BaseMsgResp, error) {
	// 获取公司 id
	...
	// 效验请求参数数据
	err = l.CheckUpdateUpgradeApkUpgradeStrategy(req)
	if err != nil {
		fmt.Println("err:", err)
	...
	// 获取策略信息
	strategyData, err := l.svcCtx.DB.UpgradeApkUpgradeStrategy.Get(l.ctx, *req.Id)
	if err != nil {
	...
	if err := base.WithTx(l.ctx, l.svcCtx.DB, func(tx *ent.Tx) error {
		...
		for i := 0; i < len(grayIds); i++ {
			err = l.svcCtx.DB.UpgradeApkUpgradeStrategyGrayStrategy.Update().
				Where(upgradeapkupgradestrategygraystrategy.IDEQ(grayIds[i])).
		...
		for i := 0; i < len(flowLimitIds); i++ {
			err = l.svcCtx.DB.UpgradeApkUpgradeStrategyFlowLimitStrategy.Update().
				Where(upgradeapkupgradestrategyflowlimitstrategy.IDEQ(flowLimitIds[i])).
				SetNotNilIsDel(&intDelTrue).
				Exec(l.ctx)
			if err != nil {
				return err
		...
		// 修改策略信息数据
		reqModel := ent.UpgradeApkUpgradeStrategy{
			ID:        *req.Id,
			CompanyID: companyID,
		}
		...
		err = l.svcCtx.DB.UpgradeApkUpgradeStrategy.UpdateOne(&reqModel).
...
func (l *UpdateUpgradeApkUpgradeStrategyLogic) CheckUpdateUpgradeApkUpgradeStrategy(req *types.UpgradeApkUpgradeStrategyInfo) error {
	// 判断是否重复
	var predicates []predicate.UpgradeApkUpgradeStrategy
	predicates = append(predicates, upgradeapkupgradestrategy.IDNEQ(*req.Id))
	...
	predicates = append(predicates, upgradeapkupgradestrategy.CompanyIDEQ(l.companyID))
