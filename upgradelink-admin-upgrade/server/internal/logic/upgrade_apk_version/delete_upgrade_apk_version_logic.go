...
func (l *DeleteUpgradeApkVersionLogic) DeleteUpgradeApkVersion(req *types.IDsReq) (*types.BaseMsgResp, error) {
	...
	for _, id := range req.Ids {
		data, err := l.svcCtx.DB.UpgradeApkVersion.Get(l.ctx, id)
		if err != nil {
		...
		// 删除状态
		predicates = append(predicates, upgradeapkupgradestrategy.ApkVersionID(id))
	...
	err = l.svcCtx.DB.UpgradeApkVersion.Update().
		Where(upgradeapkversion.IDIn(req.Ids...), upgradeapkversion.CompanyIDIn(companyID)).
